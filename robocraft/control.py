...
class Planner(object):
    def __init__(self, args, taichi_env, env_init_state, scene_params, n_particle, n_shape, model, all_p, 
        ...
        self.batch_size = min(args.shooting_batch_size, 10)
        self.sample_size = args.shooting_batch_size
    ...
    def model_rollout(
        ...
        def _predict_dynamics_ckpt(state_cur_ckpt, attrs_ckpt, Rr_curs_ckpt, Rs_curs_ckpt, Rn_curs_ckpt):
            inputs_ckpt = [attrs_ckpt, state_cur_ckpt, Rr_curs_ckpt, Rs_curs_ckpt, Rn_curs_ckpt, memory_init, group_gt, None]
            return self.model.predict_dynamics(inputs_ckpt)
        ...
        for i in range(act_seqs.shape[1]):
            ...
            for j in range(act_seqs.shape[2]):
                ...
                # pdb.set_trace()
                pred_pos, pred_motion_norm, std_cluster = checkpoint(
                    _predict_dynamics_ckpt, state_cur, attrs, Rr_curs, Rs_curs, Rn_curs
                )
